...
# Agrupando e Removendo valores duplicados
...
# Dando dimensão para a coluna e PAY_TYPE
...
# Nota-se alguns outliers muito fora da média o que pode ser creditado a equivocos 
# de digitação ou coleta. A visualização fica muito prejudicada.
# Plotando os valores em escala retirando da visualização os outliers 
# mais fora do normal.
...
# Removendo Outliers de forma a deixar apenas valores entre 
# [Q1- (1.5)IQR] e  [Q3+(1.5)IQR] e plotando 
...
# Preparando o DataSet por protudo
comp_prices_nested <- comp_prices_ajusted %>% group_by(PROD_ID) %>% nest()

# Testando por produto para verificar se o tipo de venda faz diferença nas medianas dos preços
...
# Testando por produto para verificar se a medianas dos preços e diferente por concorrente.
...
# Retirando o horário da Data das medições
comp_prices_ajusted$DATE_EXTRACTION <- as.Date(comp_prices_ajusted$DATE_EXTRACTION)

# Agrupando por data de extração e sumarisando pela média diária por produto por concorrente. 
...
# Pivotando o DataSet para trabalhar. 
...
# Adicionando lag de 1 dia, uma vez que a previsão será feita no dia anterior 
